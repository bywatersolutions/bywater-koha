...
sub wait_ready_with_logs {
    ...
    # Poll for readiness ourselves instead of calling 'ktd --wait-ready'. That waits for
    # the /ktd_ready marker file, which the 21.11 koha-testing image never writes, so it
    # always times out here even once the instance is fully up. Accept the marker when the
    # image does write it, otherwise treat a listening staff interface and message broker
    # as the end of the container's bootstrap. Bash opens the connections itself, so this
    # needs nothing installed in the container.
    my $container = "$instance_name-koha-1";
    my $probe     = qq{docker exec $container bash -c }
        . q{'[ -e /ktd_ready ] || { : </dev/tcp/localhost/8081 && : </dev/tcp/localhost/5672; }'};

    my $ready = 0;
    for ( 1 .. $timeout ) {
        my $running = qx{docker inspect -f '{{.State.Running}}' $container 2>/dev/null};
        chomp $running;

        # Give up as soon as the container dies rather than waiting out the timeout
        if ( $running eq 'false' ) {
            print "\nContainer $container has stopped or died\n";
            last;
        }

        if ( system("$probe >/dev/null 2>&1") == 0 ) {
            $ready = 1;
            last;
        }
        sleep 1;
