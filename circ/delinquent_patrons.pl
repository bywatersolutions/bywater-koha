...
if ($dateduefrom eq '' and $datedueto eq '') {
    ...
    $datedueto       = sprintf("%-04.4d-%-02.2d-%02.2d", Add_Delta_YMD($year, $month, $day, 0, 0, -56));
}
...
if ($noreport) {
    ...
    while (my $data = $sth->fetchrow_hashref) {
        ...
        my $total;
        unless (exists $balance_cache{$data->{borrowernumber}}) {
            my ( $balance, $accts, $numaccts) = GetMemberAccountRecords( $data->{borrowernumber} );
            $balance_cache{$data->{borrowernumber}} = $balance;
        }
        ...
        push @overduedata, {
            ...
            total_balance          => sprintf("%.2f", $total),
        };
    }
