...
subtest "GetHoldsQueueItems" => sub {
    ...
    my ( $queue_items, $total ) = C4::HoldsQueue::GetHoldsQueueItems();
    is( $total, $count+3,'Three items added to queue');

    ( $queue_items, $total ) = C4::HoldsQueue::GetHoldsQueueItems({itemtypeslimit => $item_1->itype});
    is( $total, 3,'Three items of same itemtype found when itemtypeslimit passed');

    ( $queue_items, $total ) = C4::HoldsQueue::GetHoldsQueueItems({itemtypeslimit => $item_1->itype, ccodeslimit => $item_2->ccode});
    is( $total, 2,'Two items of same collection found when ccodeslimit passed');

    ( $queue_items, $total ) = C4::HoldsQueue::GetHoldsQueueItems({
        itemtypeslimit => $item_1->itype,
        ccodeslimit => $item_2->ccode,
        locationslimit => $item_3->location
    });
    is( $total, 1,'One item of shleving location found when locationslimit passed');
